...
- **Observers** (registered in `AppServiceProvider::boot`):
  ...
  - `PartRequestObserver` — notifies warehouse staff (`issue_part`) when a mechanic files a parts request.
...
- **Parts workflow (real-shop model)**: a `mechanic` cannot write off stock. They file a `PartRequest` (model `PartRequest`, resource `PartRequests`) against their order; a `warehouseman` (perm `issue_part`) fulfils it via `PartRequest::fulfill()` — which decrements stock, attaches the part to the order as issued, logs a `PartMovement`, and recalculates the order total. Mechanics open their orders via a read-only `ViewOrder` page and mark only their own services done (`change_own_service_status`).
